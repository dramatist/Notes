...
SPI加载Bean：ServiceLoaderFactoryBean、ServiceFactoryBean、ServiceListFactoryBean
...
Ordered接口、@Order，定义优先级，PriorityOrdered是Ordered的扩展，实现该接口的组件永远比实现Ordered接口的组件优先级高

OrderComparator：对实现Ordered接口的对象进行排序

AnnotationAwareOrderComparator：对实现Ordered接口或标注@Order的对象进行排序
...
* 初始化完成，通常在ApplicationnContext场景使用，DefaultListableBeanFactory#preInstantiateSingletons
